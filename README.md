...
- **支持多达5个推送通道同时启用**，每个通道可独立配置，并可按短信内容正则过滤
- 支持邮件（SMTP）作为推送通道，并可按通道设置收件人和邮件模板
...
每个推送通道都可以选择配置基于 tiny-regex-c 的轻量正则过滤条件：正则为空时不限制；配置后可选择“匹配时触发”或“不匹配时触发”。支持 `.`、`^`、`$`、`*`、`+`、`?`、`[]`、`[^]`、`[a-z]`、`\s`、`\S`、`\w`、`\W`、`\d`、`\D`；不支持分组、捕获、命名捕获和 `|` 分支。
...
| **邮件（SMTP）** | 邮件通知 | 邮件通知页配置 SMTP；通道内可配置收件人和模板 |
...
- **飞书机器人**: 文本消息格式，支持加签验证
- **邮件（SMTP）**: 标题/正文模板支持`{sender}`、`{message}`、`{timestamp}`占位符；通道收件人留空时使用全局收件人
